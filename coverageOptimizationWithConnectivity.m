...
TG = [Y(:),X(:)];
sz=[Nx,Ny];
P_sink = 1; % Index position of sink
[row,col] = ind2sub(sz,P_sink);
sink = [row,col]; % 2D location of sink
Obs = [];
%Obs = [3,3;3,4;4,3;4,4];%obstacle
if ~isempty(Obs)
...
for t =1:T
    ...
    for i = 1:length(Irc_sink)
        g1(Nx*Ny +(N*T*Nx*Ny)+((t-1)*N*Nx*Ny)+ (n-1)*Nx*Ny  + Irc_sink(i)) = -1;
...
% A = [g;H;I;J;Q]; %inequality constraint LHS
% b = [(-1)*ones(size(g,1),1); zeros(size(H,1),1); zeros(size(I,1),1); zeros(size(J,1),1); zeros(size(Q,1),1)];%inequality constraint RHS
A = [E;g;H;I;J;Q]; %inequality constraint LHS
b = [ones(size(E,1),1); (-1)*ones(size(g,1),1); zeros(size(H,1),1); zeros(size(I,1),1); zeros(size(J,1),1); zeros(size(Q,1),1)];
Aineq = A;
bineq = b;
Aeq = [F;K];              %equality constraint LHS
beq = [ones(size(F,1),1); zeros(size(K,1),1)];  %equality constraint RHS
%Aeq = [R;F; K];              %equality constraint LHS
%beq = [ones(size(R,1),1);ones(size(F,1),1); zeros(size(K,1),1)];  %equality constraint RHS
%% Set the bounds
lb1= zeros(Nx*Ny,1);
...
options.timelimit= 18000; % 5hours
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% calling solver
% execution time limit = 12hrs = 43200 seconds
% options = optimoptions('intlinprog','BranchRule', 'maxpscost', 'Heuristics', 'rss',...
%      'ObjectiveImprovementThreshold', 1e-4, 'OutputFcn', 'savemilpsolutions','MaxTime', 86400);
% tic; 
